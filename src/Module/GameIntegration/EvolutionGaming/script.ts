...
export class EvolutionGamingModule implements ModuleInterface, GameInterface {
    private key: string = "evo_gaming";
    ...
    launch(options) {
        if (options.provider === this.key) {
            ...
            xhr({
                url: Router.generateModuleRoute("evolution_gaming_integration", "launch"),
                type: "json",
                method: "post",
                data: {
    ...
    private unsupportedCurrency(data) {
        xhr({
            url: Router.generateModuleRoute("evolution_gaming_integration", "unsupported"),
            type: "json",
            method: "get",
        }).then((response) => {
            if (response.status) {
