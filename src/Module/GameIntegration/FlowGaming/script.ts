...
export class FlowGamingModule implements ModuleInterface, GameInterface {
    private key: string = "flow_gaming";
    ...
    launch(options) {
        if (options.provider === this.key) {
            ...
            xhr({
                url: Router.generateModuleRoute("flowgaming_integration", "launch"),
                type: "json",
                method: "post",
                data: {
    ...
    private unsupportedCurrency(data) {
        xhr({
            url: Router.generateModuleRoute("flowgaming_integration", "unsupported"),
            type: "json",
            method: "get",
        }).then((response) => {
            if (response.status) {
