...
export class EBetModule implements ModuleInterface, GameInterface {
    private key: string = "ebet";
    ...
    launch(options) {
        if (options.provider === this.key) {
            ...
            xhr({
                url: Router.generateModuleRoute("ebet_integration", "launch"),
                type: "json",
                method: "post",
                data: {
    ...
    private unsupportedCurrency(data) {
        xhr({
            url: Router.generateModuleRoute("ebet_integration", "unsupported"),
            type: "json",
            method: "get",
        }).then((response) => {
            if (response.status) {
